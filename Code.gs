...
/**
 * STOCK CONVERSION
 * Transforms one stock item into another (e.g. cutting a larger paper sheet into smaller ones).
 * Writes paired CONVERT-OUT + CONVERT-IN ledger rows linked by conversion_no and headered by inv_conversions.
 * Source rate is carried to target (same physical paper, simplest and accurate).
 * If no target master exists, caller can supply newTarget{} to create one inline in items + inv_items.
 */
function invPostConversion(payload) {
  if (!payload) throw new Error('Conversion payload required');

  const sourceCode = String(payload.sourceItemCode || '').trim();
  const sourceQty = Number(payload.sourceQty || 0);
  const targetQty = Number(payload.targetQty || 0);
  const wasteQty = Number(payload.wasteQty || 0);
  const location = payload.location || DEFAULT_LOCATION;
  ...
  if (targetQty <= 0) throw new Error('Target qty must be positive');
  if (wasteQty < 0) throw new Error('Waste qty cannot be negative');

  const sourceItem = ensureInventoryItemExists_(sourceCode);

  let target;
  let targetMasterCreated = false;
  if (payload.targetItemCode) {
    const targetCode = String(payload.targetItemCode).trim();
    if (targetCode === sourceCode) throw new Error('Target must differ from source');
    target = ensureInventoryItemExists_(targetCode);
  } else if (payload.newTarget && payload.newTarget.itemName) {
    const created = invCreateInventoryMaster_({
      itemName: payload.newTarget.itemName,
      category: payload.newTarget.category || sourceItem.category || '',
      uom: payload.newTarget.uom || sourceItem.uom || 'NOS',
      department: payload.newTarget.department || sourceItem.department || '',
      remarks: payload.newTarget.remarks || ('Cut from ' + sourceCode),
      createdBy: payload.doneBy || ''
    });
    if (created.itemCode === sourceCode) throw new Error('Target must differ from source');
    target = (supabaseSelect('inv_items', {
      filters: { item_code: 'eq.' + created.itemCode },
      limit: 1
    }) || [])[0];
    if (!target) throw new Error('Inline master creation failed');
    targetMasterCreated = true;
  } else {
    throw new Error('Target item required (existing or new)');
  }
  const targetCode = target.item_code;
  ...
  const totalQty = allocations.reduce(function(s, a) { return s + Number(a.qty || 0); }, 0);
  const totalValue = allocations.reduce(function(s, a) { return s + Number(a.qty || 0) * Number(a.rate || 0); }, 0);
  const rate = totalQty > 0 ? Number((totalValue / totalQty).toFixed(6)) : 0;
  const consumedBatches = allocations.map(function(a) { return a.batchNo; }).filter(Boolean);
  const consumedBatchDisplay = consumedBatches.join(', ');
  const primarySourceBatch = sourceBatchNo || consumedBatches[0] || '';

  const conversionNo = generateConversionNo_();
  ...
  // 1. CONVERT-OUT ledger on source + lot deduction
  const remarksOut = [
    'Cut to ' + targetCode + ' @ ' + targetQty,
    wasteQty > 0 ? 'Waste: ' + wasteQty : '',
    remarks
  ].filter(Boolean).join(' | ');

  const ledgerOut = invAppendLedger_({
    refType: 'CONVERT-OUT',
    refNo: conversionNo,
    itemCode: sourceCode,
    qtyOut: sourceQty,
    rate: rate,
    batchNo: primarySourceBatch,
    location: location,
    remarks: remarksOut
  });

  invApplyLotIssue_(ledgerOut?.id || null, {
    itemCode: sourceCode,
    qty: sourceQty,
    location: location,
    batchNo: sourceBatchNo,
  ...
  // 2. Create new lot on target; rate carried from source FIFO average
  const sourceRefLabel = 'CONV:' + conversionNo +
    (consumedBatches.length === 1 ? (' | FROM:' + consumedBatches[0]) : '');
  const targetLot = invCreateLot_({
    ...
    location: location,
    department: payload.department || sourceItem.department || target.department || '',
    receiptDate: new Date().toISOString(),
    remarks: 'Cut from ' + sourceCode + (consumedBatches.length ? (' (batch ' + consumedBatchDisplay + ')') : '')
  });

  // 3. CONVERT-IN ledger on target
  const remarksIn = [
    'Cut from ' + sourceCode + ' @ ' + sourceQty,
    wasteQty > 0 ? 'Waste: ' + wasteQty : '',
    remarks
  ].filter(Boolean).join(' | ');

  const ledgerIn = invAppendLedger_({
    refType: 'CONVERT-IN',
    refNo: conversionNo,
    itemCode: targetCode,
    qtyIn: targetQty,
  ...
  // Resolve the newly-created lot id (invCreateLot_ returns batchNo only) for header FK
  const createdLotRow = (supabaseSelect('inv_lots', {
    select: 'id',
  ...
  // 4. Header row
  supabaseInsert('inv_conversions', {
    conversion_no: conversionNo,
    conversion_date: new Date().toISOString(),
    source_item_id: sourceItem.id,
    source_item_code: sourceCode,
    source_batch_no: primarySourceBatch || consumedBatchDisplay || '',
    source_qty: sourceQty,
    target_item_id: target.id,
    target_item_code: targetCode,
    target_batch_no: targetLot.batchNo,
    ...
    remarks: remarks,
    done_by: doneBy,
    target_master_created: targetMasterCreated,
    ledger_out_id: ledgerOut?.id || null,
    ledger_in_id: ledgerIn?.id || null,
    target_lot_id: createdLotRow?.id || null
  });
  ...
  return {
    ...
    sourceItemCode: sourceCode,
    targetItemCode: targetCode,
    targetBatchNo: targetLot.batchNo,
    targetMasterCreated: targetMasterCreated,
    rate: rate,
    consumedBatches: consumedBatches
  };
}

function invReverseConversion(conversionNo, reason) {
  ...
  const targetLot = (supabaseSelect('inv_lots', {
    ...
    filters: { id: 'eq.' + conv.target_lot_id },
    limit: 1
  }) || [])[0];
  if (!targetLot) throw new Error('Target lot missing; manual reversal required');
  if (Number(targetLot.qty_available || 0) + 0.0001 < targetQty) {
    throw new Error('Target lot already partly consumed (' +
      targetLot.qty_available + ' left of ' + targetQty + '); reversal blocked');
  }

  // Step 1: deduct target (REV-CONVERT-IN)
  const revIn = invAppendLedger_({
    refType: 'REV-CONVERT-IN',
    refNo: cnvNo,
    itemCode: conv.target_item_code,
    qtyOut: targetQty,
    rate: rate,
  ...
  supabaseUpdate('inv_lots', { id: 'eq.' + targetLot.id }, {
    qty_available: Number((Number(targetLot.qty_available || 0) - targetQty).toFixed(6))
  });
  supabaseInsert('inv_lot_allocations', {
    id: Utilities.getUuid(),
    ledger_id: revIn?.id || null,
    lot_id: targetLot.id,
    batch_no: targetLot.batch_no,
    item_id: conv.target_item_id,
    qty: targetQty,
    rate: rate,
    txn_type: 'REV-CONVERT-IN',
    created_at: new Date().toISOString()
  });

  // Step 2: restore source — new lot (REV-CONVERT-OUT)
  const restoreLot = invCreateLot_({
    itemCode: conv.source_item_code,
    qty: sourceQty,
    rate: rate,
    sourceType: 'REV-CONVERSION',
    sourceRef: 'REV:' + cnvNo,
    location: location,
    department: conv.department || '',
    receiptDate: new Date().toISOString(),
    remarks: 'Reversal restore for ' + cnvNo + ': ' + why
  });
  invAppendLedger_({
    refType: 'REV-CONVERT-OUT',
    refNo: cnvNo,
    itemCode: conv.source_item_code,
    qtyIn: sourceQty,
    rate: rate,
    batchNo: restoreLot.batchNo,
    location: location,
    remarks: 'Reversal of ' + cnvNo + ': ' + why
  });

  // Step 3: mark header reversed
  supabaseUpdate('inv_conversions', { conversion_no: 'eq.' + cnvNo }, {
...
function invCreateInventoryMaster_(payload) {
  const name = String(payload.itemName || '').trim();
  if (!name) throw new Error('Item name required for new master');
  const category = String(payload.category || '').trim();
  const uom = String(payload.uom || 'NOS').trim() || 'NOS';
  const department = String(payload.department || '').trim();
  const notes = String(payload.remarks || '').trim();

  const insertResult = supabaseInsert('items', {
    item_name: name,
    ...
    revision_controlled: false,
    item_master_notes: notes ? ('Created via stock conversion: ' + notes) : 'Created via stock conversion'
  });
  const itemRow = Array.isArray(insertResult) && insertResult.length ? insertResult[0] : insertResult;
  const itemCode = String(itemRow?.item_code || '').trim();
  if (!itemCode) throw new Error('Item master creation returned no item_code');
  ...
  return { itemCode: itemCode, itemName: name, created: true };
}

function generateConversionNo_() {
  const prefix = 'CNV';
  const seq = (supabaseSelect('inv_conversion_sequences', {
    filters: { prefix: 'eq.' + prefix },
    limit: 1
  }) || [])[0];
  let next = 1;
  if (seq) {
    next = Number(seq.last_no || 0) + 1;
    supabaseUpdate('inv_conversion_sequences', { prefix: 'eq.' + prefix }, { last_no: next });
  } else {
    supabaseInsert('inv_conversion_sequences', { prefix: prefix, last_no: 1 });
  }
  return prefix + String(next).padStart(5, '0');
}

function invListConversionsJSON(opts) {
  opts = opts || {};
  const filters = {};
  if (opts.fromDate && opts.toDate) {
    filters.and = '(conversion_date.gte.' + opts.fromDate + 'T00:00:00,conversion_date.lte.' + opts.toDate + 'T23:59:59)';
  }
  if (opts.location) filters.location = 'eq.' + opts.location;
  if (opts.sourceItemCode) filters.source_item_code = 'eq.' + opts.sourceItemCode;
  if (opts.targetItemCode) filters.target_item_code = 'eq.' + opts.targetItemCode;
  if (opts.onlyActive === true) filters.reversed_at = 'is.null';

  const rows = supabaseSelect('inv_conversions', {
    select: '*',
    filters: filters,
    order: 'conversion_date.desc',
  ...
  return {
    ...
    rows: rows.map(function(r) {
      return {
        conversionNo: r.conversion_no,
        conversionDate: r.conversion_date,
        sourceItemCode: r.source_item_code,
        sourceBatchNo: r.source_batch_no || '',
        sourceQty: Number(r.source_qty || 0),
        targetItemCode: r.target_item_code,
        targetBatchNo: r.target_batch_no || '',
        targetQty: Number(r.target_qty || 0),
        wasteQty: Number(r.waste_qty || 0),
        rate: Number(r.rate || 0),
        value: Number((Number(r.target_qty || 0) * Number(r.rate || 0)).toFixed(2)),
        location: r.location || '',
        department: r.department || '',
        cutRequestRef: r.cut_request_ref || '',
        remarks: r.remarks || '',
        doneBy: r.done_by || '',
        targetMasterCreated: r.target_master_created === true,
...
function invGetConversionDetailJSON(conversionNo) {
  const cnvNo = String(conversionNo || '').trim();
  if (!cnvNo) throw new Error('Conversion number required');
  const row = (supabaseSelect('inv_conversions', {
    filters: { conversion_no: 'eq.' + cnvNo },
    limit: 1
  }) || [])[0];
  ...
  return {
    ...
    row: {
      ...
      sourceQty: Number(row.source_qty || 0),
      targetItemCode: row.target_item_code,
      targetBatchNo: row.target_batch_no || '',
      targetQty: Number(row.target_qty || 0),
      wasteQty: Number(row.waste_qty || 0),
      rate: Number(row.rate || 0),
      value: Number((Number(row.target_qty || 0) * Number(row.rate || 0)).toFixed(2)),
      location: row.location || '',
      department: row.department || '',
      cutRequestRef: row.cut_request_ref || '',
      remarks: row.remarks || '',
      doneBy: row.done_by || '',
      targetMasterCreated: row.target_master_created === true,
      ledgerOutId: row.ledger_out_id || '',
      ledgerInId: row.ledger_in_id || '',
      targetLotId: row.target_lot_id || '',
      reversed: !!row.reversed_at,
