...
function selectArtworkJobsView_(opts) {
  const query = opts || {};
  try {
    return supabaseSelect('v_artwork_jobs', query) || [];
  } catch (e) {
    const msg = String((e && e.message) || e || '');
    const select = String(query.select || '');
    const missingTeeth = /v_artwork_jobs\.teeth/i.test(msg) && /does not exist/i.test(msg);
    if (!missingTeeth || !select) throw e;
    ...
    const rows = supabaseSelect('v_artwork_jobs', Object.assign({}, query, {
...
const DEFAULT_MASTERS = {
  ...
  departments: ['Printing','Digital','Lamination','Coating','Die Cutting','Rotary Cutting','Stitching','Side Pasting','Packing','QC','Embossing','Window Pasting','Corrugation Sheet Pasting','2 Ply Making','Cutting Pre','Cutting Post','Inspection/Slitting','Flexo Printing','Flexo Die Cutting Offline'],
  ...
  machinesByDept: {
    'Printing': ['','Heidelberg SM74', 'CD102', 'UV 2 Color Machine', 'RS4 Printing'],
    'Digital': ['','Canon Digital Printing'],
...
function _clientNormalizePayload_(payload, existing) {
  ...
  return {
    ...
    clientCode: String(existing?.client_code || src.clientCode || _clientNextCode_()).trim().toUpperCase(),
...
function mastersSaveClient(payload, token) {
  ...
  const existing = String(payload.id || '').trim()
    ...
    : null;
  _requireModuleAccess_(token, 'MASTERS', existing ? 'can_edit' : 'can_create');
...
function mastersToggleClientStatus(clientId, active, token) {
  _requireModuleAccess_(token, 'MASTERS', 'can_edit');
...
function getPlateDieJobs(opts = {}) {
  ...
  const arts = supabaseSelect('artworks', {
    select: `
      id,
      so_id,
      line_no,
  ...
  const filtered = arts.filter(a => {
    const plate = String(a.plate_status || '').trim().toUpperCase();
    const die   = String(a.die_status   || '').trim().toUpperCase();
    return plate === 'NEW' || die === 'NEW';
  });
...
if (clientCodes.size) {
  ...
  filtered.forEach(a => {
    ...
    if (!grouped[key]) {
      grouped[key] = {
        ...
        plateSize: plate.plate_size || '',
        plateCount: plate.plate_count ?? plate.count ?? '',
        hybridPlateCount: plate.hybrid_plate_count ?? '',
        hybridPlateSize: plate.hybrid_plate_size || '',
        ...
        dieCount: die.die_count ?? die.count ?? '',
  ...
  return Object.keys(grouped).map(key => {
    ...
    if (opts.pendingOnly) {
      const status = String(opts.type === 'DIE' ? row.statusDie : row.statusPlate || '').toUpperCase();
      return status !== 'RECEIVED' && status !== 'NA';
    }
...
function purchaseDashboardSummaryJSON(opts = {}) {
  ...
  arts.forEach(a => {
    ...
    if (String(raw || '').trim().toUpperCase() !== 'NEW') return;
...
function _mapWOCandidateViewRow_(r) {
  ...
  const remainingQty = Math.max(0, Number(r.remaining_qty != null ? r.remaining_qty : (qty - processedQty)) || 0);
...
function _listSOWithStatusFromCandidateView_(limit = 200, offset = 0) {
  ...
  const rawRows = supabaseSelect('v_workorder_candidates', {
    select: `
      so_number,
      so_date,
      so_time,
...
function _listSOWithStatusLegacy_(limit = 200, offset = 0, seedCandidateMap) {
  ...
  const approvalRows = _excludeFlexoWorkOrderItems_(_filterSalesServiceOnlyItems_(_excludeCancelledSalesOrdersByNumber_(rawApprovalRows, 'so_number')));
  ...
  if (seedCandidateMap && typeof seedCandidateMap === 'object') {
    ...
    let candidateRows = [];
    try {
      candidateRows = _excludeFlexoWorkOrderItems_(_filterSalesServiceOnlyItems_(_excludeCancelledSalesOrdersByNumber_(supabaseSelect('v_workorder_candidates', {
        order: 'so_number.desc,line_no.asc',
        limit: pageLimit,
        offset: pageOffset
      }) || [], 'so_number')));
  ...
  if (soNumbers.length) {
    for (let i = 0; i < soNumbers.length; i += 40) {
      ...
      (selectArtworkJobsView_({
        select: 'so_number,line_no,so_date,so_time,sales_rep,category,product_type,artwork_no,sheet_length,sheet_width,sheet_ups,printing_colors,across_ups,along_ups,total_ups,across_width,teeth,across_gap_mm,along_gap_mm,status,approved_at,accounts_status,business_status',
        filters: {
  ...
  rows.forEach(function(r) {
    ...
    const originalQty = Number(r.so_qty || r.qty || candidate.so_qty || candidate.qty) || 0;
...
function listFlexoWOStatus(limit = 200, offset = 0) {
  const cache = CacheService.getScriptCache();
  const cacheKey = 'FLEXO_WO_STATUS_' + String(limit) + '_' + String(offset);
  try {
    const cached = cache.get(cacheKey);
    if (cached) return JSON.parse(cached);
  } catch (e) {}

  const artworkRows = _filterSalesServiceOnlyItems_(_excludeCancelledSalesOrdersByNumber_(selectArtworkJobsView_({
    select: 'so_number,so_date,so_time,sales_rep,line_no,client_name,product_name,category,qty,unit,artwork_no,product_type,sheet_length,sheet_width,sheet_ups,printing_colors,across_ups,along_ups,total_ups,across_width,across_gap_mm,along_gap_mm,status,approved_at,accounts_status,business_status',
    filters: {
      product_type: 'ilike.*flexo*'
    },
  ...
  if (soIds.length) {
    _supabaseSelectByKeyInBatches_(
      ...
      jobCardMap[soNo + '||' + lineNo] = String(line.job_reference || '').trim();
    });
  }
  ...
  const data = { pending: [], processed: [], hasMore: artworkRows.length === limit };
  rows.forEach(function(r) {
    const key = String(r.so_number || '') + '||' + String(r.line_no || '');
    ...
    if (row.remainingQty > 0) data.pending.push(row);
    if (row.processedQty > 0) {
...
function getWorkOrder(woNo) {
  ...
  const routingNormalized = (snapRouting.length ? snapRouting : routingRows).map((r, index) => {
    const dbRow = routingRows[index] || {};
    return {
      dept: r.dept || r.department || dbRow.department || '',
      machine: r.machine || r.planned_machine || dbRow.planned_machine || '',
      material: r.material || '',
      out: r.out || '',
      operation: r.operation || r.process_name || dbRow.process_name || '',
...
const ERP_SESSION_TTL_SECONDS = 2 * 60 * 60;
const ERP_SESSION_TTL_MS = ERP_SESSION_TTL_SECONDS * 1000;
...
function _isSessionExpired_(session) {
  ...
  if (issuedAt > 0) return (now - issuedAt) >= ERP_SESSION_TTL_MS;
  return true;
}
...
if (hash !== user.password_hash) {
  ...
  const expiresAt = issuedAt + ERP_SESSION_TTL_MS;
...
const cache = CacheService.getScriptCache();
cache.put(token, JSON.stringify(sessionPayload), ERP_SESSION_TTL_SECONDS);
...
function getSessionUser(token){
  ...
  try {
    ...
    cache.put(token, JSON.stringify(session), ERP_SESSION_TTL_SECONDS);
...
function renderLoginRedirectPage() {
  const loginUrl = ScriptApp.getService().getUrl() + '?p=login';
  return HtmlService
    .createHtmlOutput(
      '<!DOCTYPE html><html><head><meta charset="utf-8">' +
      '<meta http-equiv="refresh" content="0;url=' + loginUrl.replace(/"/g, '&quot;') + '">' +
      '<style>' +
      'body{margin:0;font-family:Segoe UI,Arial,sans-serif;background:#f3f6fb;color:#0f172a;display:flex;align-items:center;justify-content:center;min-height:100vh;}' +
      '.card{width:min(420px,calc(100vw - 32px));background:#fff;border:1px solid #d9e2ec;border-radius:18px;box-shadow:0 18px 42px rgba(15,23,42,.12);padding:28px;}' +
      'h2{margin:0 0 8px;font-size:22px;}p{margin:0 0 18px;color:#64748b;line-height:1.5;}' +
      'a{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:0 16px;border-radius:999px;background:#e8f0ff;color:#1d4ed8;text-decoration:none;font-weight:700;border:1px solid #bfd2ff;}' +
      '</style>' +
      '<script>(function(){' +
      'var url=' + JSON.stringify(loginUrl) + ';' +
      ...
      'try{localStorage.removeItem("erp_token");}catch(e){}' +
      'try{sessionStorage.removeItem("erp_token");}catch(e){}' +
      'try{if(window.history&&typeof window.history.replaceState==="function"){window.history.replaceState({},document.title,"?p=login");}}catch(e){}' +
      'try{' +
      'var a=document.createElement("a");a.href=url;a.target="_top";a.rel="noopener";a.style.display="none";' +
      '(document.body||document.documentElement).appendChild(a);a.click();a.remove();return;' +
      '}catch(e){}' +
      'try{if(window.top&&window.top!==window&&window.top.location){window.top.location.replace(url);return;}}catch(e2){}' +
      'try{window.location.replace(url);return;}catch(e3){}' +
      'window.location.href=url;' +
      '})();</script>' +
      '</head><body><div class="card"><h2>Redirecting to Login</h2><p>Your session is no longer active. If this page does not move automatically, use the button below.</p><a href="' + loginUrl.replace(/"/g, '&quot;') + '">Go to Login</a></div></body></html>'
    )
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
...
function getArtworkJobs() {
  ...
  const rows = _filterSalesServiceOnlyItems_(_excludeCancelledSalesOrdersByNumber_(supabaseSelect(
    'v_artwork_jobs',
    {
...
function getArtworkWorkbench(fromDate, toDate, pendingOnly) {
  ...
  if (pendingOnly === true) {
    filters.status = 'neq.APPROVED';
  }

  const rows = _filterSalesServiceOnlyItems_(_excludeCancelledSalesOrdersByNumber_(selectArtworkJobsView_({
  ...
  const filteredJobs = pendingOnly
    ? jobs.filter(j => String(j.status || '').toUpperCase() !== 'APPROVED')
    : jobs;
...
function getArtworkReference(artworkNo) {
  ...
  if (!rows.length) {
    const woJob = (supabaseSelect('work_order_jobs', {
      select: 'artwork_no,category',
      filters: { artwork_no: 'eq.' + no },
      limit: 1
    }) || [])[0];
    if (!woJob) {
    ...
    return {
      artworkNo: no,
      legacyOnly: true,
      productType: _artworkNormalizeProductType_(woJob.category || ''),
      plateStatus: '',
      dieStatus: '',
  ...
  const status = statuses.length && statuses.every(function(s){ return s === 'APPROVED'; })
    ? 'APPROVED'
    : (statuses.some(function(s){ return s === 'PENDING_APPROVAL'; }) ? 'PENDING_APPROVAL' : (row.status || 'NO_ART'));
  ...
  return {
    ...
    plateCount: row.plate_count == null ? '' : Number(row.plate_count),
    hasHybridPlate: row.has_hybrid_plate === true,
    hybridPlateSize: row.hybrid_plate_size || '770X1030',
    hybridPlateCount: row.hybrid_plate_count == null ? '' : Number(row.hybrid_plate_count),
    dieCount: row.die_count == null ? '' : Number(row.die_count),
    sheetLength: row.sheet_length == null ? '' : Number(row.sheet_length),
    sheetWidth: row.sheet_width == null ? '' : Number(row.sheet_width),
    printingColors: row.printing_colors || '',
    acrossUps: row.across_ups == null ? '' : Number(row.across_ups),
    alongUps: row.along_ups == null ? '' : Number(row.along_ups),
    totalUps: row.total_ups == null ? '' : Number(row.total_ups),
    acrossWidth: row.across_width == null ? '' : Number(row.across_width),
    teeth: row.teeth == null ? '' : Number(row.teeth),
    acrossGapMm: row.across_gap_mm == null ? '' : Number(row.across_gap_mm),
    alongGapMm: row.along_gap_mm == null ? '' : Number(row.along_gap_mm),
    status: status,
    artworkAt: row.artwork_at || null,
    approvedAt: row.approved_at || null,
...
function saveArtworkGroup(payload) {
  ...
  try {
    ...
    const artworkNo = String(payload.artworkNo || '').trim() || (sequenceCandidate = _getNextArtworkNumberCandidate_(payload.productType)).artworkNo;
    const now = new Date().toISOString();
    ...
    const selectedIdSet = {};
    selectedJobs.forEach(function(job) { selectedIdSet[job.id] = true; });

    const currentRows = supabaseSelect('artworks', {
    ...
    return { ok: true, artworkNo: artworkNo };
  } finally {
    if (lockHeld) {
      try { lock.releaseLock(); } catch (e) {}
    }
...
function _opsLoadLifecycleRows_(params) {
  ...
  const rows = Object.keys(baseKeys).map(function(key) {
    ...
    const producedQty = finalRouting
      ? (function() {
    ...
    const row = {
      ...
      soId: line.so_id || packRow.so_id || so.id || '',
      soLineId: lineId || packRow.so_line_id || '',
      soNumber: soNumber,
      lineNo: primaryJob.line_no || packRow.line_no || line.line_no || '',
      client: clientMap[String(so.client_code || '')] || primaryJob.client_name || so.client_code || '',
      product: line.product_name || primaryJob.product_name || packRow.product_name || '',
      productCode: line.product_code || packRow.product_code || '',
      category: line.category || primaryJob.category || '',
...
function _opsEnrichChunkRows_(rows) {
  ...
  chunkRows.forEach(function(row) {
    ...
    row.artworkNo = primaryJob.artwork_no || '';
    row.woNumber = primaryWo?.wo_number || '';
...
function _opsResolveDatasetRows_(stage, fastRows, params, token) {
  ...
  (fastRows || []).forEach(function(row) {
    fastKeyMap[_opsRowIdentity_(row)] = true;
  });
  const missingFallbackRow = fallbackRows.some(function(row) {
    return !fastKeyMap[_opsRowIdentity_(row)];
  });
  return missingFallbackRow || fallbackRows.length > (fastRows || []).length
    ? fallbackRows
    : fastRows;
}
...
function packGetDataset(params, token) {
  ...
  try {
    const fastRows = _opsFilterStageRows_(_opsMapPackingFastRows_(_excludeCancelledSalesOrdersByNumber_(supabaseSelect('v_packing_queue_fast', {
      select: 'pack_id,so_id,so_line_id,so_number,line_no,product_code,product_name,client_name,category,department_category,order_qty,produced_qty,packed_qty,ready_to_dispatch,expected_delivery,final_delivery,division,quote_no,pm_code,product_remarks,prepress_remarks,so_remarks,artwork_no,wo_number,wo_date,transport_mode',
    ...
    Logger.log('packGetDataset fast view fallback -> ' + e.message);
  }
  const rows = _opsBuildStageDataset_('PACKING', token);
  const filtered = _opsFilterStageRows_(rows, 'PACKING', p);
  return {
    ok: true,
    rows: filtered,
    summary: _opsStageDatasetSummary_(filtered, 'PACKING')
  };
}
...
function dispatchGetDataset(params, token) {
  ...
  if (cached) return JSON.parse(cached);
  try {
    const fastRows = _opsFilterStageRows_(_opsMapDispatchFastRows_(_excludeCancelledSalesOrdersByNumber_(supabaseSelect('v_dispatch_queue_fast', {
    ...
    const rows = _opsResolveDatasetRows_('DISPATCH', fastRows, p, token);
    const result = { ok: true, rows: rows, summary: _opsStageDatasetSummary_(rows, 'DISPATCH') };
    _prodCachePutJsonSafe_(cache, cacheKey, result, 60);
    return result;
  } catch (e) {
    Logger.log('dispatchGetDataset fast view fallback -> ' + e.message);
  }
  const rows = _opsBuildStageDataset_('DISPATCH', token);
  const filtered = _opsFilterStageRows_(rows, 'DISPATCH', p);
  return {
    ok: true,
    rows: filtered,
    summary: _opsStageDatasetSummary_(filtered, 'DISPATCH')
  };
}
...
function opsLifecycleSaveInline(payload, token) {
  ...
  if (stage === 'PACKING') {
    ...
    const result = supabaseUpsertMinimal('packing_records', [{
      ...
      ready_to_dispatch: p.readyToDispatch === true,
...
function refreshStockMV_(){

  const cache = CacheService.getScriptCache();

  if(cache.get('mv_refresh_lock')) return;
...
function invPostPurchaseReceiptBulk(payload) {
  ...
  const grnNo = 'GRN' + Utilities.formatDate(new Date(), Session.getScriptTimeZone() || 'Asia/Kolkata', 'yyyyMMddHHmmss');
  const posted = [];
...
function fgGetDashboard(params, token) {
  ...
  const detailRows = _fgBuildOpeningDetailRows_().concat(_fgBuildPackedDetailRows_()).filter(function(row) {
...
function _billingBuildPrintUrl_(invoiceId, token, documentType) {
  const base = ScriptApp.getService().getUrl();
  const page = _billingNormalizeDocumentType_(documentType) === 'CHALLAN' ? 'printchallan' : 'printinvoice';
...
function previewInvoicePDF(invoiceId, token) {
  _billingRequireSession_(token);
  if (!invoiceId) throw new Error('InvoiceId required');
  let inv = {};
  try {
  ...
  return _billingBuildPrintUrl_(invoiceId, token, inv.document_type || inv.invoice_no || '');
}
...
function _prodQueueVersion_() {
  return PropertiesService.getScriptProperties().getProperty('PROD_QUEUE_VERSION') || '0';
}
...
function _prodGetStageRowsFastResult_(params) {
  ...
  const rows = _prodMapStageRowsFastViewRows_(supabaseSelect('v_production_stage_rows_fast', {
...
function _prodGetStageRowsForWoFast_(woId) {
  return _prodMapStageRowsFastViewRows_(supabaseSelect('v_production_stage_rows_fast', {
    select: 'row_key,row_kind,row_description,plan_unit,wo_id,wo_date,routing_id,wo_number,so_numbers,so_number_display,so_number,line_no,job_reference,job_ups,client_name,product_name,product_names,artwork_no,artwork_nos,process_name,process_display_name,department,planned_machine,sequence_no,expected_delivery,job_priority,department_category,planned_qty,produced_qty,balance_qty,status',
...
function _getStage1PlannedQtyFromSnapshot_(snapshot) {
...
function _prodStageStatusFromQty_(routingStatus, plannedQty, producedQty) {
  ...
  return produced > 0 ? 'IN_PROGRESS' : (status || 'PENDING');
}
...
function _prodGetPlannedAndProducedForEntry_(payload, routing, routingList, snapshot, jobs, combinedProducedTotals, combinedTotals, jobProducedTotals, jobTotals) {
  ...
  const stage1Plan = _getStage1PlannedQtyFromSnapshot_(snapshot || {});
  const splitIndex = list.findIndex(function(row) { return _isDieCutProductionStage_(row); });
  ...
  if (prev && Number(prev.sequence_no || 0) === splitSeq) {
    plannedQty = Number(combinedTotals[String(prev.id)] || 0) * ups;
  } else if (prev) {
...
function _prodBuildStageRowsFromData_(woIds, workOrderMap, routingRows, jobsByWoId, categoryByWoId) {
  ...
  ids.forEach(function(woId) {
    ...
    const stage1Plan = _getStage1PlannedQtyFromSnapshot_(snapshot);
    const splitIndex = routingList.findIndex(function(row){ return _isDieCutProductionStage_(row); });
    ...
    routingList.forEach(function(routing, idx) {
      ...
      out.push({
        ...
        rowKind: 'COMBINED',
        planUnit: _prodNormalizeCategoryGroup_(categoryByWoId[woId]) === 'FLEXO' ? 'RM' : 'SHEETS',
...
function _selectInBatches_(table, select, key, values, order) {
  ...
  const out = [];
  const chunks = _supabaseChunkValuesByFilterLength_(list, 1200, 40);
  chunks.forEach(function(chunk) {
    const rows = supabaseSelect(table, {
      select: select,
      filters: (function() {
  ...
  return boardRows.map(function(r) {
    ...
    const status = routingStatus === 'SHORT_CLOSED'
      ...
      : (planned <= 0 && produced <= 0
        ? 'PENDING'
        : (balance <= 0 ? 'COMPLETED' : (routingStatus || 'IN_PROGRESS')));
...
function saveProductionBulk(entries, token) {
  ...
  const routingRows = routingIds.length
    ? (supabaseSelect('work_order_routing', {
        select: 'id,wo_id,sequence_no,process_name,department,planned_machine,status',
        filters: { id: 'in.(' + routingIds.join(',') + ')' }
      }) || [])
    : [];
  const routingMap = {};
  ...
  const workOrders = woIds.length
    ? (supabaseSelect('work_orders', {
        select: 'id,snapshot_json',
        filters: { id: 'in.(' + woIds.join(',') + ')' }
      }) || [])
    : [];
  const workOrderMap = {};
  ...
  if (woIds.length) {
    allRoutingRows = supabaseSelect('work_order_routing', {
      select: 'id,wo_id,sequence_no,process_name,department,planned_machine,status',
      filters: { wo_id: 'in.(' + woIds.join(',') + ')' }
    }) || [];
  }
  const routingByWoId = {};
  allRoutingRows.forEach(function(row) {
  ...
  const workOrderJobs = woIds.length
    ? (supabaseSelect('work_order_jobs', {
        select: 'wo_id,so_number,line_no,product_name,qty,ups,group_ups,job_reference',
        filters: { wo_id: 'in.(' + woIds.join(',') + ')' }
      }) || [])
    : [];
  const jobsByWoId = {};
  ...
  if (inserts.length) {
    try {
      supabaseBulkInsertMinimal('production_entries', inserts);
    } catch (err) {
      const msg = String(err && err.message || '');
      ...
      supabaseBulkInsertMinimal('production_entries', inserts.map(function(row) {
        const copy = Object.assign({}, row);
...
function _reportsNormalizeFilters_(params) {
  ...
  const out = {
    ...
    fromTime: String(p.fromTime || '').trim(),
    toDate: String(p.toDate || '').trim(),
    toTime: String(p.toTime || '').trim(),
...
function _reportsApplyDateTimeFilterToQuery_(filters, columnName) {
  ...
  const key = String(columnName || '').trim();
  if (!key) return {};
  const fromIso = _reportsIsoFromFilterParts_(f.fromDate, f.fromTime, false);
  const toIso = _reportsIsoFromFilterParts_(f.toDate, f.toTime, true);
  if (fromIso && toIso) {
    const out = {};
...
function _reportsDateTimePasses_(value, filters) {
  ...
  const fromIso = _reportsIsoFromFilterParts_(filters.fromDate, filters.fromTime, false);
  const toIso = _reportsIsoFromFilterParts_(filters.toDate, filters.toTime, true);
  if (fromIso) {
...
function _reportsPlanningRows_(filters) {
  return _reportsTrySelect_('v_report_planning_lines', {
    filters: _reportsApplyDateTimeFilterToQuery_(filters, 'so_datetime'),
    ...
    return pendingPass &&
      _reportsDateTimePasses_(row.soDateTime || row.soDate, filters) &&
